...
export default function OwnerMembersPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const path = qs ? `/api/owner/members?${qs}` : "/api/owner/members";
      const result = await apiFetch<MemberListItem[]>(path, { method: "GET" }, token);
      setMembers(result);
    } catch (err) {
                ...
                {members.map((m) => (
                  <tr key={m.user_public_id} className="border-t border-border hover:bg-surface2">
                    <td className="px-3 py-2 align-top">
                      <Link
                        href={`/owner/members/${m.user_public_id}`}
