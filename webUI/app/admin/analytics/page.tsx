    ...
    <AdminShell>
      <div className="space-y-6">
        <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
          <div>
            <h2 className="text-[22px] font-semibold tracking-[-0.02em] text-text">Platform Analytics</h2>
            <p className="text-text-2">GMV, take rate, signups, and geography across tenants.</p>
          </div>
        ...
        {platform ? (
          <>
            ...
            <Card className="p-4">
              <div className="mb-3 text-sm font-semibold text-text">GMV & platform earnings (daily)</div>
              <LineTimeSeries
            ...
            <Card className="p-4">
              <div className="mb-3 text-sm font-semibold text-text">Signups (daily)</div>
              <BarByCategory
        ...
        <Card className="p-4">
          <div className="mb-3 flex items-center justify-between">
            <div className="text-sm font-semibold text-text">Top organizations by GMV</div>
            <button
              type="button"
              onClick={() =>
                downloadExport(`/api/analytics/admin/org-leaderboard?${range}&limit=50&format=csv`, "org-leaderboard.csv")
              }
              className="rounded-sm border border-line px-2 py-1 text-xs text-text-2 hover:bg-surface-2"
            >
              CSV
            </button>
          </div>
        ...
        <Card className="p-4">
          <div className="mb-3 text-sm font-semibold text-text">Bookings by city</div>
              ...
              <tbody>
                {(cities?.rows ?? []).map((c) => (
                  <tr key={c.city} className="border-t border-line">
                    <td className="p-2 text-text">{c.city}</td>
                    <td className="p-2 text-right text-text">{c.bookings}</td>
                    <td className="p-2 text-text-2">{c.lat?.toFixed(4) ?? "—"}</td>
                    <td className="p-2 text-text-2">{c.lng?.toFixed(4) ?? "—"}</td>
                  </tr>
                ))}
                {!cities?.rows.length ? (
                  <tr>
        ...
        <Card className="p-4">
          <div className="mb-3 text-sm font-semibold text-text">Downloadable reports</div>
          <div className="flex flex-wrap gap-2">
            {(["reconciliation", "failed-payments", "payouts"] as const).map((kind) => (
              <div key={kind} className="flex items-center gap-1 rounded-sm border border-line px-2 py-1 text-xs">
                <span className="text-text-2">{kind}</span>
                <button
                  type="button"
                  onClick={() =>
