...
export function OwnerCalendarScreen() {
  ...
  // Like web: preload the org's locations for the filter, auto-select a sole location.
  useEffect(() => {
    if (!token) return;
    apiFetch<Organization[]>("/api/orgs", { method: "GET" }, token)
      .then(async (orgs) => {
        ...
        for (const org of orgs) {
          const locs = await apiFetch<LocationOption[]>(
      ...
      .then((locs) => {
        setLocations((current) => (locs.length ? mergeLocations(locs, []) : current));
        if (locs.length === 1) {
          setLocationPublicId(locs[0].public_id);
        }
      })
      .catch(() => null);
  }, [token]);
      ...
      <View style={styles.headerRow}>
        <View>
          ...
          <Text style={styles.subtitle}>
            Bookings, requests, and active memberships across your locations.
          </Text>
        </View>
        <TouchableOpacity
      ...
      {locations.length > 0 ? (
        <View style={styles.filterBlock}>
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.filterRow}>
            <TouchableOpacity
              testID="owner-calendar-location-all"
            ...
            {locations.map((location) => {
              ...
              return (
                <TouchableOpacity
                  ...
                  testID={`owner-calendar-location-${location.public_id}`}
          ...
          events.map((event) => {
            ...
            return (
              <TouchableOpacity
                ...
                testID={`owner-calendar-event-${event.public_id}`}
                style={[styles.eventCard, eventTone(event.status)]}
                disabled={!openable}
                onPress={() =>
                  openable ? navigation.navigate("BookingDetail", { bookingId: event.public_id }) : null
                }
                accessibilityLabel={`Open event ${event.public_id}`}
              >
                ...
                <View style={styles.eventBody}>
                  ...
                  <View style={styles.statusRow}>
                    ...
                    {event.plan_name ? <Text style={styles.statusText}>{event.plan_name}</Text> : null}
                  </View>
                </View>
              </TouchableOpacity>
            );
