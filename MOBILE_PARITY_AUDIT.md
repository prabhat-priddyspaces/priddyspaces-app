...
| `/owner/marketing` + `/campaigns` | `owner/OwnerMarketingScreen` + `owner/OwnerMarketingCampaignsScreen` | parity *(Run 25, 2026-06-11)* | hub with per-entity counts + sender lane; campaigns: draft creation (template/segment/lane/schedule), review breakdown, send now/schedule, stats rows. Remaining 6 marketing routes (campaign detail, segments, templates, workflows, suppressions, settings): in progress, next runs |
