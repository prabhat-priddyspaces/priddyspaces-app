...
| `/owner/locations/floor-plan` | `owner/OwnerFloorPlanScreen` | parity *(Run 32, 2026-06-11)* | **discovery correction: web has NO canvas** — its page ships a 'Konva/Fabric integration goes here' placeholder; real web capabilities are presign upload, versioned plan list, numeric marker form + list. Mobile matches all of them and additionally renders the plan image. Entry: 'Floor plan' button on location cards |
...
| `/owner/marketing` + `/campaigns` | `owner/OwnerMarketingScreen` + `owner/OwnerMarketingCampaignsScreen` | parity *(Run 25, 2026-06-11)* | hub with per-entity counts + sender lane; campaigns: draft creation (template/segment/lane/schedule), review breakdown, send now/schedule, stats rows. campaign detail done Run 26 (stats, review, recipients with suppression reasons, send-now/cancel with web's status gating). segments done Run 27 (full filter builder + preview-count + save with web's filters object). templates done Run 28 (full editor: HTML/text bodies, all 9 shortcode groups, preview with missing values, test-send). workflows done Run 29 (ordered step-list builder emitting web's graph JSON — ↔ native pattern replacing the drag canvas; publish/activate/pause; runs). suppressions done Run 30; settings done Run 31 (sender lane + default verified sender via web's PUT, verified-sender requests + status refresh). **All 8 marketing routes now at parity.** |
...
- **2026-06-11 — Run 31: marketing settings.** New `OwnerMarketingSettingsScreen` mirroring `webUI/app/owner/marketing/settings/page.tsx`: lane/cap summary, default sender lane + default verified sender saved via web's `PUT /api/marketing/settings/sender` payload, verified-sender requests, and per-sender status refresh. 3 new tests (40 suites / 154 tests green). **The entire 8-route marketing suite is now at parity on mobile.**

- **2026-06-11 — Run 30: marketing suppressions.** New `OwnerMarketingSuppressionsScreen` mirroring `webUI/app/owner/marketing/suppressions/page.tsx`: add suppression (email + reason chips) with web's payload, suppressed-recipient list with reason/status/date, and resubscribe for active entries. 3 new tests (39 suites / 151 tests green).

- **2026-06-11 — Run 29: marketing workflows.** New `OwnerMarketingWorkflowsScreen` mirroring `webUI/app/owner/marketing/workflows/page.tsx`: workflow list, name + trigger-segment selection, ordered step builder (wait/send_email/condition/notify_owner/add_tag/stop with web's per-kind data defaults) emitting web's exact graph JSON as a linear chain (↔ native step list replaces ReactFlow's drag canvas — verified by graph-payload test), save (POST/PUT), publish/activate/pause actions, and the runs list. 3 new tests (38 suites / 148 tests green).

- **2026-06-11 — Run 28: marketing templates (full editor).** New `OwnerMarketingTemplatesScreen` mirroring `webUI/app/owner/marketing/templates/page.tsx`: template list with edit-on-tap, create/edit form (name/category/subject, HTML + text body editors with web's default scaffold), all nine shortcode groups appending to the active body (↔ append-at-end instead of caret insertion — RN TextInput), server-rendered preview with missing-value warnings, and test-send with optional recipient. 4 new tests (37 suites / 145 tests green).

- **2026-06-11 — Run 27: marketing segments.** New `OwnerMarketingSegmentsScreen` mirroring `webUI/app/owner/marketing/segments/page.tsx`: the full filter builder (member status, search, tags + any/all mode, subscription presence, min bookings, min lifetime revenue, last-booking-after) deriving web's exact `filters` object, preview-count POST, segment save, and the saved-segment list. 2 new tests (36 suites / 141 tests green).

- **2026-06-11 — Run 26: campaign detail.** New `OwnerMarketingCampaignDetailScreen` mirroring `webUI/app/owner/marketing/campaigns/[public_id]/client.tsx`: eligible/sent/opened/clicked stats, review summary, recipient list with statuses + suppression reasons + provider IDs, and Send now / Cancel with web's exact status gating (send only draft/scheduled; cancel blocked for sent/canceled). 4 new tests (35 suites / 139 tests green).

- **2026-06-11 — Run 25: marketing hub + campaigns.** New `OwnerMarketingScreen` (hub: per-entity counts + default sender lane, navigation cards) and `OwnerMarketingCampaignsScreen` (web's draft form with template/segment/sender-lane selection and optional schedule, review-breakdown card with Send now / Schedule using web's payloads, campaign list with stats and detail navigation). Owner menu "Marketing". 4 new tests (34 suites / 135 tests green). Remaining marketing routes follow one per run; floor-plan FULL editor and marketing content editors confirmed in-scope by owner.
