...
from app.schemas.org_customer import (
    OrgCustomerDetail,
    OrgCustomerListItem,
    OrgCustomerStats,
    OrgCustomerUpdate,
...
def _stats_to_schema(stats: MemberStats) -> OrgCustomerStats:
...
def _materialize(
    ...
    record = (
        ...
        .filter(
            OrgCustomer.organization_id == organization.id,
            OrgCustomer.user_id == user.id,
        )
        .first()
    )
    if record:
...
@router.get("/owner/members", response_model=list[OrgCustomerListItem])
def list_members(
    organization_public_id: Optional[str] = Query(None),
    status: Optional[str] = Query(None, description="Filter by OrgCustomer.status"),
    ...
    rows: dict[int, OrgCustomer] = {
        record.user_id: record
        for record in db.query(OrgCustomer)
        .filter(OrgCustomer.organization_id == organization.id)
        .all()
    }
...
@router.get("/owner/members/{user_public_id}", response_model=OrgCustomerDetail)
def get_member(
    user_public_id: str,
    organization_public_id: Optional[str] = Query(None),
    db: Session = Depends(get_db),
    ...
    if actor.role == UserAppRole.CUSTOMER:
        raise HTTPException(status_code=403, detail="Owner role required")
...
@router.patch("/owner/members/{user_public_id}", response_model=OrgCustomerDetail)
def update_member(
    user_public_id: str,
    payload: OrgCustomerUpdate,
    organization_public_id: Optional[str] = Query(None),
    db: Session = Depends(get_db),
    token: dict = Depends(get_current_user),
):
