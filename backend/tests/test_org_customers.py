...
def _seed_user(db, *, email: str, sub: str, name: str = "Customer X") -> User:
    user = User(
        email=email,
        auth_subject=sub,
...
def test_list_members_includes_request_only_lead(db_session, client_factory):
    ...
    customer = _seed_user(db_session, email="lead@example.com", sub="sub-lead", name="Lead Person")
    db_session.add(
        BookingRequest(
    ...
    assert len(data) == 1
    item = data[0]
    assert item["user_public_id"] == customer.public_id
    assert item["materialized"] is False  # No OrgCustomer row written yet.
...
def test_member_detail_lazy_materializes_and_aggregates(db_session, client_factory):
    ...
    customer = _seed_user(db_session, email="ali@example.com", sub="sub-ali", name="Ali")
    ...
    client = client_factory(_owner_token(owner))
    resp = client.get(f"/api/owner/members/{customer.public_id}")
    assert resp.status_code == 200
    data = resp.json()
    ...
    # PATCH lazily creates the OrgCustomer row and persists fields.
    resp2 = client.patch(
        f"/api/owner/members/{customer.public_id}",
    ...
    resp3 = client.get(f"/api/owner/members/{customer.public_id}")
...
def test_customer_role_forbidden_on_members_list(db_session, client_factory):
    cust = _seed_user(db_session, email="cust-only@example.com", sub="sub-cust-only")
    client = client_factory(_owner_token(cust))
    resp = client.get("/api/owner/members")
