...
def test_booking_request_approval_rechecks_conflicts(db_session, client_factory):
    owner, space = _seed_owner_space(db_session)
    customer = User(
        email="cust-conflict@example.com",
        auth_subject="sub-cust-conflict",
        role=UserAppRole.CUSTOMER,
        email_verified=True,
        is_active=True
    )
    ...
    end = datetime(2026, 2, 5, 12, 0, tzinfo=timezone.utc)
    customer_client = client_factory({
        "sub": "sub-cust-conflict",
        "email": customer.email,
        "email_verified": True
    })
